$(document).ready(function() {
	...
	const tblTiposRecompensas = $('#tblTiposRecompensas').DataTable({
		// Configuración inicial
		lengthMenu: [[5, 10, -1], [5, 10, "Todos"]],
		pageLength: 10, 
		dom: "Blifrtp", //B:buttons f:filter r:processing t:table
		...
		buttons: [
			{   extend: "excelHtml5",
				text: "<i class='fa-solid fa-file-excel'></i>",
				titleAttr: "Exportar todas las filas a excel", //tooltip
				filename: `Club de Técnicos DIMACOF-Listado de Tipos de Recompensas-${obtenerFechaHoraFormateadaExportaciones()}`, 
				title: 'Club de Técnicos DIMACOF-Listado de Tipos de Recompensas', 
				action: function (e, dt, node, config) {
					// Guardar el valor actual de `pageLength`
					const currentPageLength = tblTiposRecompensas.page.len();
	
					// Cambiar a mostrar todas las filas
					tblTiposRecompensas.page.len(-1).draw(); 
					loadingModal.classList.add('show');

					setTimeout(() => {
						loadingModal.classList.remove('show');
						$.fn.dataTable.ext.buttons.excelHtml5.action.call(this, e, dt, node, config);
						tblTiposRecompensas.page.len(currentPageLength).draw(); 
					}, 500); // Esperar medio segundo para asegurar que todas las filas estén cargadas
				},
			},  
			{   extend: "pdfHtml5",
				...
				customize: function(doc) {
					// Centrar título
					doc.content[0].text = {
						text: 'Club de Técnicos DIMACOF - Listado de Tipos de Recompensas ',
						bold: true,
						alignment: 'center'
					};
				},
				action: function (e, dt, node, config) {
					window.open(baseUrlMAIN + '/dashboard-tiposRecompensas/export-pdf', '_blank');
				}
			},  
		],
	...
	window.tblSolicitudesAppCanje  = $('#tblSolicitudesAppCanje').DataTable({
		...
		"createdRow": function(row, data){
			let tdEstadoSolicitudCanje = row.children[6];
			let span = document.createElement('span');
		...
		*/
		...
		columns: [
			...
			},
			{
				data: null, // Indica que no se tomará un solo campo
				render: function (data) {
					return `${data.idVentaIntermediada} <br> <small> Fecha emisión: ${data.fechaHoraEmision_VentaIntermediada} </small><br>
			...
			},
			{
				data: null,
				render: function (data) {
					return `<small>Venta: </small>${data.diasTranscurridosVenta} <br>
							<small>Solicitud: </small>${data.diasTranscurridos_SolicitudCanje}`;
				},
				name: 'diasTranscurridosVenta_diasTranscurridosSolicitud', // Nombre para búsquedas
			},
